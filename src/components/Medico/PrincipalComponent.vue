      ...
      <div class="grid">
        ...
        > -->
          <h4>
            ...
            <i
              style="cursor:pointer"
              @click="this.$router.push(`/medicos/new`)"
              class="fas fa-plus"
              _mstvisible="2"
            ></i>
          </h4>
          <ListadoComponent :medicos="medicos" v-show="!cargando" />
        </div>
      </div>
    </div>
  </div>
...
import ListadoComponent from "@/components/Medico/ListComponent.vue";
import Navbar from "@/components/Navbar.vue";
import { Medico } from "@/interfaces/Medico";
import { getMedicos } from "@/services/luiscruzrd/Medico";
...
export default {
  ...
  methods: {
    ...
    async filterMedicos(catName: string) {
      ...
      if (catName !== "Todos") {
        this.medicos = this.medicos.filter((medico: Medico) => {
          return medico.especialidad === catName;
        });
      }
    },
    ...
    async search(term: string) {
      ...
      if (term !== "Todos") {
        this.medicos = this.medicos.filter((medico: Medico) => {
          return medico.medico.toLowerCase().includes(term.toLowerCase());
        });
      }
      this.toggleLoading();
    },
...
.grid {
  ...
  grid-template-areas:
    ...
    "medicos medicos medicos medicos medicos medicos medicos";
}
