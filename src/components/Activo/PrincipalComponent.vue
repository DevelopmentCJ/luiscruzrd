      ...
      <div class="grid">
        ...
        > -->
          <h4>
            ...
            <i
              style="cursor:pointer"
              @click="this.$router.push(`/activos/new`)"
              class="fas fa-plus"
              _mstvisible="2"
            ></i>
          </h4>
          <ListadoComponent :activos="activos" v-show="!cargando" />
        </div>
      </div>
    </div>
  </div>
...
<script lang="ts">
import Filters from "@/components/Activo/FiltersComponent.vue";
import ListadoComponent from "@/components/Activo/ListComponent.vue";
import Navbar from "@/components/Navbar.vue";
import { Activo } from "@/interfaces/Activo";
import { getActivos } from "@/services/luiscruzrd/Activo";
...
export default {
  ...
  methods: {
    ...
    async filterActivos(catName: string) {
      try {
      ...
      if (catName !== "Todos") {
        this.activos = this.activos.filter((activo: Activo) => {
          return activo.categoria === catName;
        });
      }
    },
    ...
    async search(term: string) {
      ...
      if (term !== "Todos") {
        this.activos = this.activos.filter((activo: Activo) => {
          return activo.busqueda.toLowerCase().includes(term.toLowerCase());
        });
      }
      this.toggleLoading();
    },
