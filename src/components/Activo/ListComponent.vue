<template>
  <table v-if="this.totales.activos" id="customers">
    <tr>
      <th>Ubicación</th>
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/activos/${item._id}`)"
    >
      <td>{{ item.ubicacion }}</td>
      <td>{{ item.departamentoArea }}</td>
    ...
    <tr>
      <td>Total: {{ formatNumber(this.totales.activos) }}</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
  ...
  <h6 class="noRegist" v-if="!this.totales.activos">
...
import { updateOne } from "@/services/luiscruzrd/Activo";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(activo: Activo) {
      alert("Activo Lista.");
      try {
        const res = await updateOne(activo);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.activos = this.activos.length;
      this.totales.valor = this.activos.reduce(
