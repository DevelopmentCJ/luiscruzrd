<template>
  <table v-if="this.totales.productos" id="customers">
    <tr>
      <th>Descripción</th>
      <th>Referencia</th>
      <th>Código Barra</th>
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/productos/${item._id}`)"
    >
      <td>{{ item.desc }}</td>
      <td>{{ item.ref }}</td>
      <td>{{ item.barcode }}</td>
  ...
  <h6 class="noRegist" v-if="!this.totales.productos">
...
import { updateOne } from "@/services/luiscruzrd/Producto";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(producto: Producto) {
      alert("Producto Lista.");
      try {
        const res = await updateOne(producto);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.productos = this.productos.length;
      this.totales.valor = this.productos.reduce(
